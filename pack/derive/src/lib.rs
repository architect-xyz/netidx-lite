    ...
    match input {
            ...
            Fields::Named(fields) => {
                let fields = fields
                    ...
                    .map(|f| {
                        let name = &f.ident;
                        quote! {
                            pack::Pack::encoded_len(&self.#name)
                        }
                    });
                if no_wrap {
                    quote! {
            ...
            Fields::Unnamed(fields) => {
                let fields = fields
                    ...
                    .map(|(i, _)| {
                        ...
                        quote! {
                            pack::Pack::encoded_len(&self.#index)
                        }
                    });
                if no_wrap {
                    quote! {
        ...
        Data::Enum(en) => {
            ...
            if no_wrap {
                ...
                quote! {
                    pack::len_wrapped_len(1 + match self {
                        #(#cases),*
                    })
                }
            }
            ...
            Fields::Named(fields) => {
                let fields = fields
                    ...
                    .map(|f| {
                        let name = &f.ident;
                        quote! {
                            pack::Pack::encode(&self.#name, buf)?
                        }
                    });
                if no_wrap {
                    quote! {
                    ...
                    quote! {
                        pack::len_wrapped_encode(buf, self, |buf| {
                            #(#fields);*;
                            Ok(())
                        })
                    }
            ...
            Fields::Unnamed(fields) => {
                let fields = fields
                    ...
                    .map(|(i, _)| {
                        let index = Index::from(i);
                        quote! {
                            pack::Pack::encode(&self.#index, buf)?
                        }
                    });
                if no_wrap {
                    quote! {
        ...
        Data::Enum(en) => {
            ...
            let cases = en.variants.iter().enumerate().map(|(i, v)| {
                    ...
                    Fields::Named(f) => {
                        ...
                        let pack_fields = f
                            ...
                            .map(|f| {
                                let name = &f.ident;
                                quote! {
                                    pack::Pack::encode(#name, buf)?
				}
                            });
                        quote! {
                            Self::#tag { #(#match_fields),*, .. } => {
				<u8 as pack::Pack>::encode(&#i, buf)?;
				#(#pack_fields);*;
				Ok(())
                            }
			}
				...
				Ok(())
                    ...
                    Fields::Unit => {
                        quote! {
                            Self::#tag => <u8 as pack::Pack>::encode(&#i, buf),
			}
                    }
                }
            });
...
fn decode_with_default(name: &Option<Ident>) -> TokenStream {
    quote! {
        let #name = match pack::Pack::decode(buf) {
            Ok(t) => t,
            Err(pack::PackError::BufferShort) =>
    ...
    match input {
        Data::Struct(st) => match &st.fields {
            Fields::Named(fields) => {
                ...
                if no_wrap {
                    ...
                    quote! {
                        pack::len_wrapped_decode(buf, |buf| {
                            #(#decode_fields);*;
                            Ok(Self { #(#name_fields),* })
                        })
                    }
        ...
        Data::Enum(en) => {
            ...
            if no_wrap {
                match other {
                    Some(other) => quote! {
                        match <u8 as pack::Pack>::decode(buf)? {
                            #(#cases)*
                            #other
                        }
                    },
                    None => quote! {
                        match <u8 as pack::Pack>::decode(buf)? {
                            #(#cases)*
                            _ => Err(pack::PackError::UnknownTag)
                        }
                    },
                }
            } else {
...
pub fn derive_pack(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    for param in &mut input.generics.params {
        if let GenericParam::Type(typ) = param {
            typ.bounds.push(parse_quote!(pack::Pack))
        }
    }
    ...
    let expanded = quote! {
        impl #impl_generics pack::Pack for #name #ty_generics #where_clause {
